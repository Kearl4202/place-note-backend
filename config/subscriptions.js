...
// Limits come from subscriptions.js — single source of truth.
// Previously this file kept its own copy keyed by display name only, so a
// tier value of 'chief' (what the RevenueCat webhook actually writes) fell
// through to Viewer limits and archived paying users down to 5 notes.
const { SUBSCRIPTION_TIERS } = require('./subscriptions');
...
function resolveLimits(tierName, contactPackCount) {
  const tierKey = TIER_ALIASES[(tierName || '').toLowerCase()] || tierName;
  const tier = SUBSCRIPTION_TIERS[tierKey] || SUBSCRIPTION_TIERS['The Viewer'];
  const packs = tierKey === 'The Chief' ? (contactPackCount || 0) : 0;
  return { ...tier.limits, contacts: tier.limits.contacts + packs };
}
...
async function handleDowngrade(userId, newTier) {
  try {
    const { data: downgradeUser } = await supabase
      .from('users')
      .select('contact_pack_count')
      .eq('id', userId)
      .single();
    const limits = resolveLimits(newTier, downgradeUser?.contact_pack_count);
...
async function archiveCleanup() {
  try {
    ...
    const { data: graceUsers } = await supabase
      .from('users')
      .select('id, subscription_tier, contact_grace_deadline, push_token, contact_pack_count')
    ...
    if (graceUsers && graceUsers.length > 0) {
      for (const graceUser of graceUsers) {
        const limits = resolveLimits(graceUser.subscription_tier, graceUser.contact_pack_count);
